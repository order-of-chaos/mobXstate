...
- selected state side `+` controls -> `addConnectedState` draft command;
- empty canvas double-click -> positioned `addState` draft command;
- `Backspace` / `Delete` -> remove selected transition first, otherwise
...
Этот комментарий содержит только compressed JSON layout metadata. При повторном
открытии inspector читает строку после `@mobxstate`, декодирует layout и
восстанавливает позиции state nodes и transition/action labels. Комментарий не
должен менять `MachineConfig`, `DraftModel` или runtime behavior.
...
Первый реализованный вертикальный срез поддерживает `addState` в существующий
object-literal `states`: webview получает `SOURCE_PATCH_PREVIEW`, пользователь
подтверждает `SOURCE_PATCH_APPLY`, VS Code adapter применяет edits через
`WorkspaceEdit`, а source worker перечитывает файл и возвращает обновленную
machine model.
