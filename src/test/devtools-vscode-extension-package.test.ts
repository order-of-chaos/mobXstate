...
describe("VS Code extension package scaffold", () => {
  ...
  it("keeps the webview graph-first with editor and simulation modes", () => {
    ...
    expect(source).toContain("SOURCE_PATCH_PREVIEW");
    expect(source).toContain("SOURCE_PATCH_APPLY");
    expect(source).toContain("pendingSourcePatch");
    expect(source).toContain('data-testid="source-patch-preview"');
    expect(source).toContain('data-editor-command="applySourcePatch"');
    expect(source).toContain("addConnectedState");
    expect(source).toContain("state-node-add-control");
    expect(source).toContain('data-editor-command="addConnectedState"');
    expect(source).toContain("onPaneClick");
    expect(source).toContain("event.detail !== 2");
    expect(source).toContain("handleDeleteSelection");
