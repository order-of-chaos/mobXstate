...
Это MobXstate type compiler, а не compatibility wrapper для другого runtime.
Он заполняет MobXstate `TypegenMeta`.
...
Для MobXstate UI слово `services` не используется. Внутри typegen contract
`eventsCausingServices` остается alias для effects до изменения public type
contract.
